...
invisible(load_libraries("dplyr", "tibble", "tidyverse", "DT", "SummarizedExperiment", "survival", "survminer", "readr", "glmnet", "caret", "openxlsx", "biomaRt", "writexl", "edgeR", "pROC", "caTools", "survivalROC", "risksetROC", "lattice", "survMisc", "SIS", "VennDiagram", "glmSparseNet", "randomForestSRC", "grf", "Boruta", "timeROC", "qvalue", "clusterProfiler", "org.Hs.eg.db", "enrichplot", "pathview", "UpSetR","rsample", "pec"))
```
...
# Models' names
models_names_list <- c("Cox", "SIS", "ISIS", "RandomForest+Cox", "CausalForest+Cox", "Boruta+Cox")
...
# Create an empty dataframe to store Integrated Brier Score (IBS)
IBS_df <- data.frame(matrix(ncol = length(seeds), nrow = num_models))
colnames(IBS_df) <- paste0("Seed_", seeds)
rownames(IBS_df) <- models_names_list
...
for (i in seq_along(seeds)) {
  ...
  for (j in seq_along(models_coefficients_list[[i]])) {
    ...
    if (num_coeff_df[j, i] > 0) {
      ...
      IBS_df[j, i] <- calculate_integrated_brier_score(survival_object_train, genes_expression_train, models_coefficients_list[[i]][[j]], genes_expression_test, survival_test)
...
best_model_index <- which.max(rowMeans(c_index_df, na.rm = TRUE)) #4
best_model_name <- rownames(c_index_df)[best_model_index]

cat("Best model:", best_model_name, "\n")

# Get the coefficients of the best model
best_model_coefficients <- models_coefficients_list[[10]][[best_model_index]] # o index aqui escolhido tem que bater certo com o seed do split data para que dê igual performance
...
pdf("Results/riskAUC.pdf", width = 6, height = 4) # to save the plot below
...
cat("Concordance based on Incident Case/Dynamic Control definition:", riskAUC$Cindex, "\n")
...
genes_list <- list()
for (i in 1:n_seeds) {
  genes_list[[paste0("Seed: ", i)]] <- models_coefficients_list[[i]][[best_model_index]]$ensembl_gene_id
}

# Create an UpSet plot
upset_plot <- upset(fromList(genes_list),nsets = n_seeds, order.by = "degree")

# save the plot
pdf("Results/upset_plot.pdf", width = 6, height = 5, onefile=FALSE) # Create a PDF file to save the plot
upset_plot
...
# Define the seeds for the iterations
seeds_all_equal <- seq(1, n_seeds_all_equal, by = 1)

# Initialize a list to store the coefficients for each iteration
models_coefficients_list_all_equal <- list()

# Create an empty dataframe to store C-index values
num_models_all_equal <- length(models_names_list) # Assuming each iteration has the same number of models
c_index_df_all_equal <- data.frame(matrix(ncol = length(seeds_all_equal), nrow = num_models_all_equal))
colnames(c_index_df_all_equal) <- paste0("Seed_", seeds_all_equal)
rownames(c_index_df_all_equal) <- models_names_list

# Create an empty dataframe to store Integrated Brier Score (IBS)
IBS_df_all_equal <- data.frame(matrix(ncol = length(seeds_all_equal), nrow = num_models_all_equal))
colnames(IBS_df) <- paste0("Seed_", seeds_all_equal)
rownames(IBS_df) <- models_names_list

# Create an empty dataframe to store the number of coefficients
num_coeff_df_all_equal <- data.frame(matrix(ncol = length(seeds_all_equal), nrow = num_models_all_equal))
colnames(num_coeff_df_all_equal) <- paste0("Seed_", seeds_all_equal)
rownames(num_coeff_df_all_equal) <- models_names_list

# Loop through the seeds and run the analysis
for (i in seq_along(seeds_all_equal)) {
  cat("Running analysis for seed:", seeds_all_equal[i], "\n")
  ...
  for (j in seq_along(models_coefficients_list_all_equal[[i]])) {
    ...
    num_coeff_df_all_equal[j, i] <- length(models_coefficients_list_all_equal[[i]][[j]]$ensembl_gene_id)
    
    if (num_coeff_df_all_equal[j, i] > 0) {
      
      c_index_df_all_equal[j, i] <- calculate_c_index(survival_object_train, genes_expression_train, models_coefficients_list_all_equal[[i]][[j]], genes_expression_test, survival_test, survival_data)
      
      IBS_df_all_equal[j, i] <- calculate_integrated_brier_score(survival_object_train, genes_expression_train, models_coefficients_list_all_equal[[i]][[j]], genes_expression_test, survival_test)
        
      cat("seed: ", seed_all_equals[i], "\n",
          "c_index: ", c_index_df_all_equal[j, i], "\n", 
          "IBS: ", IBS_df_all_equal[j, i], "\n")
      num_coeff_df_all_equal[j, i] <- length(models_coefficients_list_all_equal[[i]][[j]]$ensembl_gene_id)
    
    } else {
      
      # If the model has empty coefficients, set C-index to NA 
      c_index_df_all_equal[j, i] <- NA
      IBS_df_all_equal[j, i] <- NA
           }
    }
}


MRR_C_Index_all_equal <- calculate_MRR(c_index_df_all_equal, higher_is_better = TRUE)
MRR_IBS_all_equal <- calculate_MRR(IBS_df_all_equal, higher_is_better = FALSE)
...
summary_table_all_equal <- data.frame(
  ...
  MRR_C_Index = MRR_C_Index_all_equal$MRR,
  IBS_Avg = round(apply(IBS_df, 1, mean, na.rm = TRUE), 3),
  IBS_Std_Dev = round(apply(IBS_df, 1, sd, na.rm = TRUE), 3),
  MRR_IBS = MRR_IBS_all_equal$MRR,
...
# Identify the model with the highest average C-index
best_model_index_all_equal <- which.max(rowMeans(c_index_df_all_equal, na.rm = TRUE)) #4
best_model_name_all_equal <- rownames(c_index_df_all_equal)[best_model_index_all_equal]
...
# Print the concordance based on the Incident Case/Dynamic Control definition (calculated as a weighted average of the area under time-specific ROC)
cat("Concordance based on Incident Case/Dynamic Control definition:", riskAUC_all_equal$Cindex, "\n")
...
#   genes_list_all_equal[[paste0("Seed: ", i)]] <- models_coefficients_list_all_equal[[i]][[best_model_index_all_equal]]$ensembl_gene_id
# }
# 
# # Create an UpSet plot
...
genes_list <- bitr(gene_info_result$ensembl_gene_id,
